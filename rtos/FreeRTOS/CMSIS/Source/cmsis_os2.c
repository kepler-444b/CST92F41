...
uint32_t osEventFlagsWait(osEventFlagsId_t ef_id, uint32_t flags, uint32_t options, uint32_t timeout)
{
    ...
    if ((hEventGroup == NULL) || ((flags & EVENT_FLAGS_INVALID_BITS) != 0U)) { // 参数有效性检查
        rflags = (uint32_t)osErrorParameter;
    } else if (IRQ_Context() != 0U) { // 是否在中断中调用
        rflags = (uint32_t)osErrorISR;
    } else {
        if (options & osFlagsWaitAll) { // 是否需要等待所有指定标志位都置位
            wait_all = pdTRUE;
        } else {
            wait_all = pdFAIL; // 只需等待任意一个指定标志位置位
        }

        if (options & osFlagsNoClear) {
            exit_clr = pdFAIL; // 返回时不自动清除已触发的标志位
        } else {
            exit_clr = pdTRUE; // 返回时自动清除已触发的标志位
        }

        // 调用 freertos 原生函数
        rflags = xEventGroupWaitBits(hEventGroup, (EventBits_t)flags, exit_clr, wait_all, (TickType_t)timeout);

        if (options & osFlagsWaitAll) { // 检查是否所有标志位都置位
            if ((flags & rflags) != flags) {
                if (timeout > 0U) {
                    rflags = (uint32_t)osErrorTimeout; // 超时
                } else {
                    rflags = (uint32_t)osErrorResource; // 资源不可用
                }
            }
        } else {
            if ((flags & rflags) == 0U) { // 检查是否有任何标志位置位
                if (timeout > 0U) {
                    rflags = (uint32_t)osErrorTimeout; // 超时
                } else {
                    rflags = (uint32_t)osErrorResource; // 资源不可用
