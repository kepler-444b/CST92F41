...
#if defined(CONTROLLER_FASTFAIR) && (CONTROLLER_FASTFAIR)
#include "remote_auth/mible_remote_auth.h"
#else
...
#elif (HAVE_CONFIRM_BUTTON==0)
	advertising_init(1);
#endif
#if defined(MI_BLE_ENABLED)
    mible_record_delete(RECORD_DFU_INFO);   //remove dfu info
#endif
    return mible_reboot();
}
 ...
 *@param    [in] slen : string len, if set to 0 use strlen() to calc (must include '/0').
 *@return   NULL: out of memory, others: pointer of value or arguments
 */
property_value_t * property_value_new_bytype(property_format_t type, void *value, uint16_t slen);
...
property_value_t * property_value_new_strn(const char *value, uint16_t slen);
 ...
 *@param    [in] queue_size : gatt queue length(default 8, minimum 8)
 *@return   0: success, negetive value: failure
 */
static inline int miio_gatt_spec_init(property_operation_callback_t set_cb,
                                    ...
                                    uint16_t buflen, uint16_t queue_size)
{
    return mible_gatt_spec_init(set_cb, get_cb, action_cb, buflen, queue_size);
}

/**
 *@brief    send properties_changed (single prop).
 *@param    [in] siid: service id.
 *@param    [in] piid: property id.
 *@param    [in] newValue: property value.
 *@return   0: success, negetive value: failure
 */
static inline int miio_gatt_property_changed(uint16_t siid, uint16_t piid, property_value_t *newValue)
{
 ...
 *@return   0: success, negetive value: failure
 */
static inline int miio_gatt_properties_changed(uint8_t nums, properties_t *newProps)
{
    return gatt_send_properties_changed(nums, newProps);
}
 ...
 *@return   0: success, negetive value: failure
 */
static inline int miio_mesh_low_power_mode(bool enable)
{
    return mible_mesh_low_power_mode(enable);
}
...
#if defined(CONTROLLER_FASTFAIR) && (CONTROLLER_FASTFAIR)
/**
*@brief    fastpair set timeout to stop adv
*@param    [in]time_ms: unit-ms
*/
static inline int miio_ble_set_fastpair_timeout(uint16_t time_ms)
{
    return set_fastpair_timeout(time_ms);
}

/**
*@brief   .force the fastrpair stopped immediately
*@param    none
*/
static inline int miio_ble_set_fastpair_force_stopped()
{
    return set_fastpair_force_stopped();
}

/**
*@brief    button object set timeout to stop adv
*@param    [in]time_ms: unit-ms
*/
static inline int miio_ble_set_object_timeout(uint16_t time_ms)
{
	return set_object_timeout(time_ms);
}
 ...
 */
static inline int miio_ble_user_callback_register(property_operation_objid_callback_t get_cb, uint16_t adv_interval_ms)
{
    return mible_user_property_get_callback_register(get_cb, adv_interval_ms);
}
 ...
 */
static inline int miio_ble_period_obj_add(uint16_t obj_id)
{
	return mibeacon_period_obj_add(obj_id);
}

/**
 *@brief    add an spec property into the period adv list
 *@param    [in]siid: the service id to be added into the list
 *@param    [in]piid: the property id to be added into the list
 */
static inline int miio_ble_period_spec_property_add(uint8_t siid, uint16_t piid)
{
	return mibeacon_period_spec_property_add(siid, piid);
}

/**
 *@brief    add an spec event into the period adv list
 *@param    [in]siid: the service id to be added into the list
 *@param    [in]eiid: the event id to be added into the list
 */
static inline int miio_ble_period_spec_event_add(uint8_t siid, uint16_t eiid)
{
	return mibeacon_period_spec_event_add(siid, eiid);
}
...
/**
 * @brief   mibeacon set power saving flag
 *
 * @param   [in] power_saving_bitmask: reduce power consume flag
 *
 * @note    bit0: stop adv after send props or event
 *
 * @note    bit1: use non-connect adv
 *
 */
static inline void miio_ble_set_power_saving(uint8_t power_saving_bitmask)
{
	mibeacon_set_object_power_saving(power_saving_bitmask);
}
